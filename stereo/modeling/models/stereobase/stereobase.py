...
from stereo.modeling.common.refinement import StereoDRNetRefinement
...
class StereoBase(nn.Module):
    def __init__(self, cfgs):
        ...
        assert 'COST' in cfgs, 'new yaml model config needs COST'
        self.num_groups = cfgs.COST.get('NUM_GROUPS', 8)
        self.use_concat_volume = cfgs.COST.get('USE_CONCAT_VOLUME', False)
        self.use_gwc_volume = cfgs.COST.get('USE_GWC_VOLUME', True)
        self.use_sub_volume = cfgs.COST.get('USE_SUB_VOLUME', False)
        self.use_interlaced_volume = cfgs.COST.get('USE_INTERLACED_VOLUME', False)
        self.concat_feature_channel = cfgs.COST.get('CONCAT_CHANNELS', 12)
        self.interlaced_feature_channel = cfgs.COST.get('INTERLACED_CHANNELS', 8)
    ...
    def forward(self, data):
        ...
        init_disp = self.rgb_refine(init_disp, image1, image2)
