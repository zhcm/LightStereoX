...
class STTR(nn.Module):
    def __init__(self, cfgs):
        ...
        self.cfgs = cfgs
        self.validation_max_disp = cfgs.VALIDATION_MAX_DISP
        self.px_threshold = cfgs.PX_ERROR_THRESHOLD
        self.downsample = cfgs.DOWNSAMPLE
        ...
        self.tokenizer = Tokenizer(block_config=[4, 4, 4, 4],
                                   ...
                                   hidden_dim=cfgs.CHANNEL_DIM,
                                   growth_rate=4)

        if cfgs.POSITION_ENCODING == 'sine1d_rel':
            self.pos_encoder = PositionEncodingSine1DRelative(cfgs.CHANNEL_DIM)
        else:
            self.pos_encoder = None

        self.transformer = Transformer(hidden_dim=cfgs.CHANNEL_DIM,
                                       nhead=cfgs.NHEADS,
                                       num_attn_layers=cfgs.NUM_ATTN_LAYERS)
        self.regression_head = RegressionHead(cfgs.REGRESSION_HEAD == 'ot')

        self.cal = ContextAdjustmentLayer(cfgs.CAL_NUM_BLOCKS,
                                          cfgs.CAL_FEAT_DIM,
                                          cfgs.CAL_EXPANSION_RATIO)
