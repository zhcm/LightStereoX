...
class LightStereo(nn.Module):
    ...
    def forward(self, data):
        ...
        bump = disp_pred * data['bump_mask'].unsqueeze(1)
        bump = self.bump_conv(bump)
        bump = self.global_avg_pool(bump).flatten(start_dim=1)
        bump = self.fc1(bump)
        bump = self.fc2(bump)
        result['bump'] = bump
    ...
    def get_loss(self, model_pred, input_data):
        ...
        return loss + bump_loss, loss_info
