...
class LightStereo(nn.Module):
    def __init__(self, backbone, max_disp, aggregation_blocks, expanse_ratio, left_att=True):
        ...
        self.data_classification = nn.Sequential(
            nn.Conv2d(272, 80, kernel_size=1, padding=1, bias=False),
            nn.BatchNorm2d(80),
            nn.ReLU(),
            nn.AdaptiveAvgPool2d(1),
            nn.Flatten(1),
            nn.Linear(80, 6, bias=True)
        )
    ...
    def forward(self, data):
        ...
        if self.training:
            ...
            logits = self.data_classification(features_left[-1])
    ...
    def get_loss(self, model_pred, input_data):
        ...
        classifier_loss = nn.CrossEntropyLoss()(model_pred['logits'], input_data['data_class'])
        loss += 0.5 * classifier_loss

        loss_info = {'scalar/train/loss_disp': loss.item(),
                     'scalar/train/loss_classifier': 0.5 * classifier_loss.item()}
