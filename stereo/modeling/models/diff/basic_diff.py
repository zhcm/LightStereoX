...
class IgevNoGruDiff(nn.Module):
    def __init__(self, cfgs):
        ...
        self.diff_classify = nn.Conv3d(volume_channel, 1, 3, 1, 1, bias=False)
    ...
    def forward(self, data):
        ...
        gwc_volume = build_gwc_volume(match_left, match_right, self.max_disp // 4, 8)  # [bz, channel, max_disp/4, H/4, W/4]

        diff_cond = self.diff_classify(gwc_volume).squeeze(1)
        diff_filters = self.pipeline(cond_feature=diff_cond, num_inference_steps=5)
        volume_filter = (diff_filters[-1] / 2 + 0.5).clamp(0, 1).unsqueeze(1)
        ...
        disp_up = context_upsample(init_disp * 4., spx_pred.float()).unsqueeze(1)  # bz, 1, h, w
        res_dict = {'disp_pred': disp_up}
        ...
        if self.training:
            ...
            noise_pred = self.diff_model(noisy_images, timesteps, diff_cond)
            res_dict['noise'] = noise
            res_dict['noise_pred'] = noise_pred

            diff_disp_preds = []
            for diff_filter in diff_filters[:-1]:
                volume_filter = (diff_filter / 2 + 0.5).clamp(0, 1).unsqueeze(1)
                filted_volume = gwc_volume * volume_filter
                ...
                spx_pred = F.softmax(spx_pred, 1)  # bz, 9, h, w
                disp_up = context_upsample(init_disp * 4., spx_pred.float()).unsqueeze(1)  # bz, 1, h, w
                diff_disp_preds.append(disp_up)

            res_dict['disp_diff_preds'] = diff_disp_preds
    ...
    def get_loss(self, model_pred, input_data):
        ...
        disp_pred_loss = 1.0 * F.smooth_l1_loss(disp_pred[valid.bool()], disp_gt[valid.bool()], reduction='mean')
        loss_info['scalar/train/disp_pred'] = disp_pred_loss.item()

        # diff middle result loss
        loss_gamma = 0.9
        disp_diff_preds = model_pred['disp_diff_preds']
        n_predictions = len(disp_diff_preds)
        assert n_predictions >= 1
        disp_diff_loss = 0
        for i in range(n_predictions):
            adjusted_loss_gamma = loss_gamma ** (15 / (n_predictions - 1))
            i_weight = adjusted_loss_gamma ** (n_predictions - i - 1)
            i_loss = (disp_diff_preds[i] - disp_gt).abs()
            disp_diff_loss += i_weight * i_loss[valid.bool()].mean()
        loss_info['scalar/train/disp_diff_loss'] = disp_diff_loss.item()

        # diff norise pred loss
        ...
        # kl loss
        # pred_prob = model_pred['pred_prob']
        # disp_gt_prob = model_pred['disp_gt_prob']
        # loss_pointwise = disp_gt_prob * (torch.log(disp_gt_prob) - torch.log(pred_prob)) * model_pred['prob_loss_mask']
        # kl_loss = 1000 * loss_pointwise.mean()
        # loss_info['scalar/train/kl_loss'] = kl_loss.item()

        loss = disp_pred_loss + disp_diff_loss + loss_mse
