...
from ..lightstereo.repvit import repvit_m0_6
from stereo.modeling.backbones.mpvit import mpvit_tiny
...
class RepvitFeature(SubModule):
    def __init__(self):
        super(RepvitFeature, self).__init__()
        model = repvit_m0_6()
...
class MpvitFeature(SubModule):
    def __init__(self):
        super(MpvitFeature, self).__init__()
        model = mpvit_tiny()
        checkpoint = load_state_dict_from_url('mpvit_tiny.pth', map_location='cpu')
        state_dict = checkpoint['model']
