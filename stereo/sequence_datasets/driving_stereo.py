...
class DrivingStereo(SequenceDatasetTemplate):
    def __init__(self, data_root_path, augmentations, logger, sample_len):
        super().__init__(data_root_path, augmentations, logger)
        self.sample_len = sample_len

        all_seqs = sorted(glob(os.path.join(data_root_path, 'train-left-image/*')))
        for each_seqs in all_seqs:
            left_images = sorted(glob(os.path.join(each_seqs, '*.jpg')))
    ...
    def _append_sample(self, left_images):
        ...
        for ref_idx in range(0, seq_len - self.sample_len):
            ...
            for idx in range(ref_idx, ref_idx + self.sample_len):
                ...
                sample["image"]['right'].append(left_images[idx].replace('train-left-image', 'train-right-image'))
                sample["disparity"]['left'].append(left_images[idx].replace('train-left-image', 'train-disparity-map').replace('.jpg', '.png'))
    ...
    def __getitem__(self, index):
        ...
        for i in range(sample_size):
            ...
            depth = np.load(sample["disparity"]['left'][i])
