...
class VirtualKitti2(SequenceDatasetTemplate):
    def __init__(self, data_root_path, augmentations, logger, sample_len):
        ...
        for each_seqs in all_seqs:
            left_images = sorted(glob(os.path.join(each_seqs, 'frames/rgb/Camera_0/*.jpg')))
    ...
    def _append_sample(self, left_images):
        ...
        for ref_idx in range(0, seq_len - self.sample_len):
            ...
            for idx in range(ref_idx, ref_idx + self.sample_len):
                ...
                sample["image"]['right'].append(left_images[idx].replace('Camera_0', 'Camera_1'))
                sample["disparity"]['left'].append(left_images[idx].replace('rgb', 'depth').replace('.jpg', '.png'))
                sample["disparity"]['right'].append(left_images[idx].replace('rgb', 'depth').replace('Camera_0', 'Camera_1').replace('.jpg', '.png'))
