...
train_augmentations = [
    ...
    LazyCall(stereo_trans.RandomSparseScale)(crop_size=[288, 1152], min_pow_scale=-0.2, max_pow_scale=0.4, prob=0.8),
    LazyCall(stereo_trans.KittiRandomCrop)(crop_size=[288, 1152]),
...
# scheduler
max_iter = 39000
scheduler = LazyCall(OneCycleLR)(optimizer=None, max_lr=lr, total_steps=max_iter, pct_start=0.05,
                                 cycle_momentum=False, anneal_strategy='cos')

clip_grad = LazyCall(ClipGradNorm)(max_norm=1.0)

# runtime params max_iter=39000, all_batchsize=4, epoch=500000/55350, lr=0.0002
runtime_params.save_root_dir = os.path.join(project_root_dir, 'output/KittiDataset/NMRF')
